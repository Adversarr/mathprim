...
#include <mathprim/linalg/iterative/precond/ic_cusparse_ext.hpp>
...
template <typename Flt = float>
static std::tuple<index_t, double, double> pcg_with_ext_cholesky(
    const Eigen::SparseMatrix<Flt, Eigen::RowMajor>& A,        //
    nb::ndarray<Flt, nb::ndim<1>, nb::device::cpu> b,          //
    nb::ndarray<Flt, nb::ndim<1>, nb::device::cpu> x,          //
    const Eigen::SparseMatrix<Flt, Eigen::RowMajor>& lower_l,  //
    const Flt& rtol,                                           //
    index_t max_iter,                                          //
    int verbose) {
  using SparseBlas = mp::sparse::blas::cusparse<Flt, sparse::sparse_format::csr>;
  using LinearOp = SparseBlas;
  using Blas = mp::blas::cublas<Flt>;
  using Precond = mp::sparse::iterative::cusparse_ichol_ext<Flt, device::cuda, sparse::sparse_format::csr>;
  using Solver = mp::sparse::iterative::cg<Flt, mp::device::cuda, LinearOp, Blas, Precond>;
  ...
  // 3. Setup preconditioner with external matrix.
  auto start = time_now();
  Solver solver(view_device.as_const());
  
  // Set the external preconditioner matrix
  auto ainv_host = eigen_support::view(lower_l);
  solver.preconditioner().derived().set_external(ainv_host);
...
static void bind_extra(nb::module_& m) {
  ...
  m.def("pcg_with_ext_cholesky", &pcg_with_ext_cholesky<Flt>,
        "Preconditioned CG on GPU (cpu->gpu->cpu) (with Cholesky precond.)",
        nb::arg("A").noconvert(),        // System to solve
        nb::arg("b").noconvert(),        // Right-hand side
        nb::arg("x").noconvert(),        // Initial guess
        nb::arg("lower_l").noconvert(),  // Lower triangular matrix from Cholesky factorization
        nb::arg("rtol") = 1e-4f,         // Relative tolerance
        nb::arg("max_iter") = 0,         // Maximum iterations (0 means rows of A)
        nb::arg("verbose") = 0);         // Verbosity
}
