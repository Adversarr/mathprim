...
void work_cuda_ic_ext(benchmark::State &state) {
  int dsize = state.range(0);
  sparse::laplace_operator<Scalar, 2> lap(make_shape(dsize, dsize));
  auto h_mat_buf = lap.matrix<mathprim::sparse::sparse_format::csr>();
  auto d_mat_buf = h_mat_buf.to<device::cuda>();
  ...
  using linear_op = sparse::blas::cusparse<Scalar, sparse::sparse_format::csr>;
  using blas_t = blas::cublas<Scalar>;
  using preconditioner = sparse::iterative::cusparse_ichol_ext<Scalar, device::cuda, sparse::sparse_format::csr>;
  sparse::iterative::cg<Scalar, device::cuda, linear_op, blas::cublas<Scalar>, preconditioner> cg{mat};

  Eigen::IncompleteCholesky<Scalar, Eigen::Lower, Eigen::NaturalOrdering<index_t>> ichol;
  ichol.compute(eigen_support::map(h_mat_buf.view()));

  if (ichol.info() != Eigen::Success) {
    state.SkipWithError("Eigen Incomplete Cholesky failed");
  }
...
BENCHMARK(work_cuda_ic_ext)->Range(1 << 4, LARGE_RANGE)->Unit(benchmark::kMillisecond);
