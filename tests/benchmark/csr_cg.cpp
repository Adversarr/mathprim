...
static void work2(benchmark::State &state) {
  ...
  using linear_op
      = iterative_solver::sparse_matrix<sparse::blas::eigen<float, sparse::sparse_format::csr>>;
  using preconditioner
      = iterative_solver::diagonal_preconditioner<float, device::cpu, sparse::sparse_format::csr, blas_impl>;
  iterative_solver::cg<float, device::cpu, linear_op, blas_impl, preconditioner> cg{linear_op{mat}, blas_impl{},
                                                                                    preconditioner{mat}};
...
BENCHMARK_TEMPLATE(work2, blas::cpu_handmade<float>)->Range(1 << 10, 1 << 16);
BENCHMARK_TEMPLATE(work2, blas::cpu_blas<float>)->Range(1 << 10, 1 << 16);
BENCHMARK_TEMPLATE(work2, blas::cpu_eigen<float>)->Range(1 << 10, 1 << 16);
