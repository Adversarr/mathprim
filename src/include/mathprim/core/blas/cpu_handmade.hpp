...
template <typename T>
struct backend_blas<T, blas_t::cpu_handmade, device_t::cpu> {
...
template <typename T>
MATHPRIM_BACKEND_BLAS_COPY_IMPL(T, blas_t::cpu_handmade, device_t::cpu) {
  for (auto sub : dst.shape()) {
    dst(sub) = src(sub);
  }
}

template <typename T>
MATHPRIM_BACKEND_BLAS_SCAL_IMPL(T, blas_t::cpu_handmade, device_t::cpu) {
...
template <typename T>
MATHPRIM_BACKEND_BLAS_SWAP_IMPL(T, blas_t::cpu_handmade, device_t::cpu) {
  for (auto sub : x.shape()) {
...
template <typename T>
MATHPRIM_BACKEND_BLAS_AXPY_IMPL(T, blas_t::cpu_handmade, device_t::cpu) {
  for (auto sub : x.shape()) {
    y(sub) += alpha * x(sub);
  }
}
...
template <typename T>
MATHPRIM_BACKEND_BLAS_DOT_IMPL(T, blas_t::cpu_handmade, device_t::cpu) {
  T v = 0;
  for (auto sub : x.shape()) {
    v += y(sub) * x(sub);
  }
...
template <typename T>
MATHPRIM_BACKEND_BLAS_NORM_IMPL(T, blas_t::cpu_handmade, device_t::cpu) {
  T v = 0;
  for (auto sub : x.shape()) {
    v += x(sub) * x(sub);
  }
...
template <typename T>
MATHPRIM_BACKEND_BLAS_ASUM_IMPL(T, blas_t::cpu_handmade, device_t::cpu) {
  T v = 0;
  for (auto sub : x.shape()) {
    v += std::abs(x(sub));
  }
...
template <typename T>
MATHPRIM_BACKEND_BLAS_AMAX_IMPL(T, blas_t::cpu_handmade, device_t::cpu) {
  T v = 0;
  for (auto sub : x.shape()) {
    T xi = std::abs(x(sub));
    v = xi > v ? xi : v;
